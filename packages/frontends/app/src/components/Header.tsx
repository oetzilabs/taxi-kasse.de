...
export const [headerVariant, setHeaderVariant] = createSignal<HeaderVariants[keyof HeaderVariants]>("default");
...
export const Header = () => {
  ...
  return (
    <nav
      class={cn("flex items-center sticky top-0 z-50 bg-transparent justify-between flex-wrap w-full", {
        "hidden !h-0": !visibleHeader(),
        "bg-white dark:bg-black": headerVariant() === HeaderVariants.DEFAULT,
      })}
    >
      <Switch>
        <Match when={headerVariant() === HeaderVariants.DEFAULT}>
          <div
            class={cn(" flex items-center justify-between flex-wrap transition-[width]", {
              "w-full p-8": headerVariant() === HeaderVariants.MAP,
              "container mx-auto py-8 md:px-0 px-4": headerVariant() === HeaderVariants.DEFAULT,
            })}
          >
                      ...
                      <Switch
                        fallback={
                          <Switch
                            fallback={
                              <A
                                href={`${import.meta.env.VITE_AUTH_URL}/authorize?${authUrl.toString()}`}
                                rel="noreferrer"
                              >
        ...
        <Match when={headerVariant() === HeaderVariants.MAP}>
          <div></div>
        </Match>
      </Switch>
    </nav>
