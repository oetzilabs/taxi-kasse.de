...
const EventsMap = <T extends Events.Info>(props: EventClientMapProps<T>) => {
  ...
  onMount(() => {
    if (mapContainer) {
      const origin = props.event().origin;
      if (!origin) return;
  ...
  createEffect(() => {
    ...
    markers = [L.marker([Number(event.origin!.latitude), Number(event.origin!.longitude)], {})];
