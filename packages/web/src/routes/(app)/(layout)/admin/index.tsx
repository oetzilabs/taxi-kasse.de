...
import { getAuthenticatedAdminSession } from "@/lib/auth/util";
...
export const route = {
  preload: async () => {
    ...
    return { notification, rides, session, stats, hotspot, companies };
  },
...
export default function ADminDashboardPage() {
  const session = createAsync(() => getAuthenticatedAdminSession(), { deferStream: true });
  const stats = createAsync(() => getSystemStatistics(), { deferStream: true });
  const rides = createAsync(() => getSystemRides(), { deferStream: true });
