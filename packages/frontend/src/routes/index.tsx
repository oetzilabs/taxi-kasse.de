...
function UserDashboard(props: UserDashboardProps) {
  ...
  return (
    <Show
      when={stats() && stats()}
      fallback={
        <div class="flex flex-col w-full justify-center items-center gap-8 py-10">
          <div class="flex text-xl">You are currently not connected to a Company</div>
          <div class="flex flex-col">
            <A
              href="/company/connect"
              class="bg-white dark:bg-black border border-neutral-100 dark:border-neutral-900 rounded-sm px-2 py-1 hover:bg-neutral-100 dark:hover:bg-neutral-900 active:bg-neutral-200 dark:active:bg-neutral-800 focus:outline-none focus:ring-2 focus:ring-neutral-100 dark:focus:ring-neutral-900"
            >
              Connect Company
            </A>
          </div>
...
export default function Home() {
  const [user] = useAuth();
  return (
    <div class="flex container mx-auto flex-col gap-4">
      <Show when={!user().isLoading && user().isAuthenticated && user()}>
        {(user) => <UserDashboard user={user().user!} token={user().token!} />}
      </Show>
      <Show when={!user().isAuthenticated}>
        <div class="flex flex-col gap-4">
          <h1 class="text-2xl font-bold">Welcome to Taxi Kasse</h1>
          <p class="text-lg">Taxi Kasse is a simple app to help you keep track of your cab drives.</p>
          <p class="text-lg">
            Taxi Kasse is open source, so you can see how it works, and you can contribute to make it better.
          </p>
        </div>
      </Show>
    </div>
